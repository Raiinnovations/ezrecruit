...
const SolutionCard = ({ solution, delay }: { solution: { heading: string }; delay: number }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-card rounded-xl overflow-hidden flex flex-col flex-shrink-0 border border-primary/30 shadow-lg w-[180px] md:w-[280px]"
    >
      {/* Solution Heading */}
      ...
      <div className="relative bg-background flex-1 p-1.5 md:p-2">
        <img
          ...
          className="w-full object-cover object-top rounded-md h-[70px] md:h-[120px]"
        />
      </div>
    </motion.div>
  );
    ...
    >
      ...
      <div className="rounded-2xl border border-border/50 bg-muted/30 shadow-xl overflow-hidden h-full flex flex-col">
        ...
        <div className="p-4 md:p-6 bg-gradient-to-br from-background to-muted/20 flex-1 flex flex-col min-h-0">
          ...
          {/* Solution Section */}
          <SolutionSection stepData={stepData} animationKey={animationKey} solutionIntro={stepData.solutionIntro} />
        </div>
      </div>
    </motion.div>
  );
...
// Solution cards section (no carousel)
const SolutionSection = ({ stepData, animationKey, solutionIntro }: { stepData: typeof stepsData[0]; animationKey: number; solutionIntro: string }) => {
  return (
    <>
      <motion.div
